...
#include <Message.h>
#include <private/app/appserverconfig.h>
...
WindowPanel::WindowPanel( const BRect& cFrame ) : LayoutView( cFrame, "", NULL, B_FOLLOW_NONE )
{
    ...
    m_pcDecoratorList    = new BListView( BRect(0,0,0,0), "decorator_list", BListView::F_RENDER_BORDER | BListView::F_NO_AUTO_SORT );
    m_pcPenMenu          = new CDropDownList( BRect(0,0,0,0), "pen_list" );
    m_pcColorEdit        = new ColorEdit( BRect(0,0,0,0), "color_edit", ui_color(B_PANEL_BACKGROUND_COLOR) );
    m_pcPopupWndCheckBox = new BCheckBox( BRect(0,0,0,0), "all_popup_selected_windows", "Popup selected windows", NULL );
    ...
    pcRoot->ExtendMaxSize( BPoint( 0.0f, FLT_MAX ) );
    pcRightPanel->ExtendMaxSize( BPoint( 0.0f, FLT_MAX ) );
    pcTopRightPanel->ExtendMaxSize( BPoint( FLT_MAX, FLT_MAX ) );
    pcBottomRightPanel->ExtendMaxSize( BPoint( FLT_MAX, FLT_MAX ) );
    ...
    pcRoot->SetBorders( BRect(5.0f,5.0f,5.0f,5.0f) );
    pcRoot->SetBorders( BRect(5.0f,15.0f,5.0f,10.0f), "decorator_list", "top_left_panel", NULL );
    pcRoot->SetBorders( BRect(5.0f,10.0f,5.0f,10.0f), "bottom_left_panel", NULL );
    pcRoot->SetBorders( BRect( 15.0f,10.0f,0.0f,0.0f), "refresh_but", "apply_but", "close_but", NULL );
    ...
    m_pcPenMenu->SetSelectionMessage( new BMessage( ID_PEN_CHANGED ) );
  
    m_pcDecoratorList->SetInvokeMsg( new BMessage( ID_APPLY ) );
...
void WindowPanel::FrameResized( float inWidth, float inHeight )
{
...
void WindowPanel::KeyDown( const char *bytes, int32 numBytes )
{
    switch( bytes[0] )
    {
        case B_UP_ARROW:
...
void WindowPanel::Draw( BRect cUpdateRect )
{
...
void WindowPanel::UpdateDecoratorList()
{
    ...
    strcat( pzPath, "/drivers/appserver/decorators" );
...
void WindowPanel::MessageReceived( BMessage* pcMessage )
{
    switch( pcMessage->what )
    {
        ...
        case ID_APPLY:
        {
            const char* pzBaseDir;
            char zPath[1024];
      
            AppserverConfig().SetPopoupSelectedWindows( m_pcPopupWndCheckBox->Value() );
            ...
            be_app->CommitColorConfig();
            BStringItem* pcRow = static_cast<BStringItem*>(m_pcDecoratorList->GetRow( nSelection ));
            pzBaseDir = getenv( "COSMOE_SYS" );
            if( pzBaseDir == NULL )
            {
                pzBaseDir = "/cosmoe";
            }
            strcpy( zPath, pzBaseDir );
            strcat( zPath, pcRow->GetString( 0 ).c_str() );
            be_app->SetWindowDecorator( zPath );
            break;
        }
        default:
