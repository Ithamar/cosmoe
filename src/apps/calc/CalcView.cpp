...
void CalcView::ChangeHighlight()
{
	...
	{
		rgb_color on = {204,204,255,255};
		rgb_color off = {204*9/10, 204*9/10, 255*9/10, 255};
...
BView * CalcView::AddExtra()
{
	...
	r.bottom = r.top + 16;
	r.right = r.left + 8+2;
	FrameView *binary_frame = new FrameView(r, "", 0, 1);
	...
	r.right = r.left + 72;
	binary_frame = new FrameView(r, "", 0, 1);
	view->AddChild(binary_frame);
	r2 = binary_frame->Bounds();
	r2.InsetBy(2,2);
	r2.OffsetBy(1,1);
	...
	r.bottom = r.top + 16;
	r.InsetBy(-1,0);
	FrameView *about_frame = new FrameView(r, "", 0, 1);
	view->AddChild(about_frame);
	r2 = about_frame->Bounds();
	r2.InsetBy(2,2);
	r2.OffsetBy(0,1);
	BStringView *peter = new BStringView(r2, "",
...
{
	//dbprintf("CalcView::KeyDown: got %d bytes (%s)\n", numBytes, bytes);
	...
	for (int i=0; i<numBytes; i++)
	{
		...
		if (button != NULL)
		{
			//dbprintf("CalcView::KeyDown: sending message!\n");
...
static void DoubleToBinary(double *dPtr, char *str)
{
	...
	if (((unsigned char *)&endian_test)[0] != 0)	// little-endian
	{
...
void CalcView::About()
{
	//new BAlert("About Calculator", "Calculator\n\n  Brought to you by Peter Wagner.\n  Copyright 1998.","OK")->Go();
	BAlert* anAlert = new BAlert("About Calculator",
								"Calculator\n\n  Brought to you by Peter Wagner.\n  Copyright 1998.",
								"OK");
...
*/
		...
		default:
			...
			{
				//dbprintf("CalcView::MessageReceived(): sending keystroke\n");
