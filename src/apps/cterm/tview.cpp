...
uint8        g_anPalette[] =
{
    ...
    0xff, 0xff, 0x00,                // Light brown
    0xff, 0xff, 0xff                 // white
};

int g_nDefaultAttribs = 0x60;

rgb_color g_cSelectColor = { 200, 100, 200, 255 };
...
static rgb_color GetAttrFgColor( int nAttrib )
{
    ...
    int        nInd = nAttrib & 0x0f;

    sColor.red   = g_anPalette[ nInd * 3 + 0 ];
    sColor.green = g_anPalette[ nInd * 3 + 1 ];
    sColor.blue  = g_anPalette[ nInd * 3 + 2 ];
  
    return( sColor );
}


static rgb_color GetAttrBgColor( int nAttrib )
{
    rgb_color sColor;

    int        nInd = (nAttrib >> 4) & 0x0f;
...
TextLine::TextLine( char nChar, int nAttrib )
{
    int        i;
    ...
    m_pCharMap           = new char[ m_nMaxLength ];
    m_pAttribMap         = new int16[ m_nMaxLength ];

    for ( i = 0 ; i < m_nMaxLength ; ++i )
	{
        m_pAttribMap[i] = nAttrib;
    }
        
    memset( m_pCharMap, nChar, m_nMaxLength );
}
...
void TextLine::Clear( int nStart, int nEnd, char nChar, int nAttr )
{
    int        i;
    if ( nStart < 0 )
	{
...
/*
void TextLine_t::Compress()
{
}

void        TextLine_t::Expand( int nAttribs, int nLength )
{
        if ( nLength != m_nSize )
        {
                char*                pzNewCharBuf         = new char[ nLength ];
                int16*        pzNewAttrBuf        =        new int16[ nLength ];
                
                for ( i = 0 ; i < nLength ; ++i )
                {
                        if ( i < m_nSize ) {
                                pzNewCharBuf[i] = m_pCharMap[i];
                                pzNewAttrBuf[i] = m_pAttribMap[i];
                        } else {
                                pzNewCharBuf[i]         = ' ';
                                pzNewAttrBuf[i] = nAttribs;
                        }
                }
                delete[] m_pCharMap;
                delete[] m_pAttribMap;
                        
                m_pCharMap                 = pzNewCharBuf;
                m_pAttribMap        =        pzNewAttrBuf;
                m_nSize = nLength;
        }
}
*/
...
/*
void        TextLine::InsertChar( int nPos, char nChar, int nAttribs, bool bOverwrite )
{
}

void        TextLine::RemoveChar( int nPos )
{
}

void        TextLine::Clear( int nStart, int nEnd )
{
        int        i;
        for ( i = nStart ; i <= nEnd ; ++i ) {
                m_pCharMap[ i ] = ' ';
                m_pAttribMap[ i ] = m_nCurAttrib;
        }
}
...
TermView::TermView( BRect cFrame, const char* pzTitle, uint32 nResizeMask, uint32 nFlags )
                : BView( cFrame, pzTitle, nResizeMask, nFlags )
{
    m_cMaxCharMapSize = IPoint( 0, 0 );
    m_cCurCharMapSize = IPoint( 0, 0 );

    m_nScrollTop     = 0;
    m_nScrollBottom  = -1;        // Force initializing in FrameResized()
...
{
    if ( y1 < 0 )                         y1 = 0;
    if ( y2 >= m_cCurCharMapSize.y )        y2 = m_cCurCharMapSize.y - 1;
...
TextLine* TermView::GetLineBuf( int y, bool bWidthScroll ) const
{
    ...
    {
//    printf( "ERROR : GetLineBuf() - Attempt to get line %d. Height = %d TLC=%d Off=%d\n",
//              y, m_cCurCharMapSize.y, m_nTotLineCnt, m_nScrollPos );
        return( NULL );
    }
...
TextLine* TermView::GetPrevLineBuf( int y ) const
{
    ...
    {
        printf( "ERROR : GetPrevLineBuf() - Attempt to get line %d. Height = %d\n", y, m_cCurCharMapSize.y );
        return( NULL );
        y = 0;
    }
...
void TermView::AttachedToWindow( void )
{
    ...
    m_cCharSize.x = StringWidth( "M" );
    m_cCharSize.y = m_sFontHeight.ascent + m_sFontHeight.descent + m_sFontHeight.leading;
    MakeFocus( true );
}
...
void TermView::ExpandCharMap( IPoint cNewSize )
{
	...
	if ( cNewSize.y > 0 /* m_cCurCharMapSize.y */ )
	{
		if ( cNewSize.y > m_nTotLineCnt/*m_nMaxLineCount*/ )
		{
...
void TermView::Copy()
{
    ...
    for ( int y = cPnt1.y ; y <= cPnt2.y ; ++y )
	{
		...
		{
            if ( isspace( pcLineBuf->m_pCharMap[ x ] ) == false )
			{
                nMaxX = x;
                break;
            }
        }
        cBuffer.append( pcLineBuf->m_pCharMap + nMinX, nMaxX - nMinX + 1 );
        if ( y != cPnt2.y )
		{
            cBuffer += '\n';
        }
    }
    BClipboard cClipboard("tempclip");
    cClipboard.Lock();
    cClipboard.Clear();
    BMessage* pcData = cClipboard.Data();
...
void TermView::MouseDown( BPoint cPosition )
{
	int32       nButtons = 0x01;
	...
	m_cSelectStart = cPosition - BPoint( 0, m_nScrollPos * m_cCharSize.y );
	m_cSelectEnd   = m_cSelectStart;
	m_bIsSelRangeValid = true;
}


void TermView::MouseUp( BPoint cPosition )
{
	m_bIsSelecting = false;
	...
	const char* pzStr;
	int i;
	for ( i = 0 ; pcData.FindString( "file/path", i, &pzStr ) == 0 ; ++i )
	{
		std::string cBuffer;
...
void TermView::SortSelection( IPoint* pcPnt1, IPoint* pcPnt2 )
{
	...
	*pcPnt1 = PixToChar( cPnt1 );
	*pcPnt2 = PixToChar( cPnt2 );
}
...
void TermView::MouseMoved( BPoint cNewPos, uint32 nCode, const BMessage* pcData )
{
	...
	RemoveSelection();
	m_cSelectEnd = cNewPos - BPoint( 0, m_nScrollPos * m_cCharSize.y );
	HighlightSelection();
	m_bModified = true;
}


void TermView::Draw( BRect cUpdateRect )
{
    RefreshDisplay( true );
}
...
{
    rgb_color sBgCol = GetAttrBgColor( nAttribs );
    ...
    StrokeLine( BPoint( m_cCsrPos.x * m_cCharSize.x, y * m_cCharSize.y + 1 ),
                BPoint( m_cCsrPos.x * m_cCharSize.x, y * m_cCharSize.y + m_cCharSize.y -1 )  );
        
    StrokeLine( BPoint( m_cCsrPos.x * m_cCharSize.x + 1, y * m_cCharSize.y + 1 ),
                BPoint( m_cCsrPos.x * m_cCharSize.x + 1, y * m_cCharSize.y + m_cCharSize.y -1 ) );
}
...
{
    int16   nLastAttrib = pcLine->m_pAttribMap[x1]; // m_pAttribMap[ x1 + y * m_cMaxCharMapSize.x ];
    int     nStartX = x1;
    int     x;

    for( x = x1 ; x <= x2 ; ++x )
    {
//                int         nCharOffset =  x + y * m_cMaxCharMapSize.x;
        int16 nNextAttrib = pcLine->m_pAttribMap[x + 1]; // m_pAttribMap[ nCharOffset + 1 ];
        ...
        if ( x != x2 ) {
            if ((nNextAttrib & (ATTR_INVERSE | ATTR_SELECTED)) != (nLastAttrib & (ATTR_INVERSE | ATTR_SELECTED))) {
                ...
                bStateChange = (nLastAttrib & ATTR_INVERSE) ?
                    (nNextAttrib & ATTR_FG_MASK) != (nLastAttrib & ATTR_FG_MASK) :
                    (nNextAttrib & ATTR_BG_MASK) != (nLastAttrib & ATTR_BG_MASK);
            }
        } else {
            bStateChange = true;
        }
        ...
        if ( bStateChange ) {
            if ( (nLastAttrib & ATTR_SELECTED) == 0 ) {
                SetHighColor( (nLastAttrib & ATTR_INVERSE) ?
                            GetAttrFgColor( nLastAttrib ):
                            GetAttrBgColor( nLastAttrib ) );
            } else {
                SetHighColor( g_cSelectColor );
            }

            FillRect( BRect( nStartX * m_cCharSize.x, y * m_cCharSize.y,
                            (x + 1) * m_cCharSize.x - 1,
                            y * m_cCharSize.y + m_cCharSize.y ) );
            nStartX = x + 1;
...
void TermView::RenderLinePart( TextLine* pcLine, int x1, int x2, int y, int nAscender, bool bOnlyModified )
{
    ...
    for( x = x1 ; x <= x2 ; ++x ) {
        int16 nNextAttrib = pcLine->m_pAttribMap[x + 1]; //  m_pAttribMap[ nCharOffset + 1 ];
        char  nChar = pcLine->m_pCharMap[y]; // m_pCharMap[ nCharOffset ];
        bool  bDoRender = false;
        int   nSpaceCnt = 0;

        if ( nNextAttrib != nLastAttrib || x == x2 )
        {
            if ( ' ' == nChar && ((nLastAttrib & ATTR_UNDERLINE) == 0) ) {
//                                nSpaceCnt++;;
            } else {
                if ( nSpaceCnt > 2 ) {
                    bDoRender = true;
        ...
        if ( bDoRender )
        {
            ...
            if ( x >= nStartX )
            {
                if ( (nLastAttrib & ATTR_SELECTED) == 0 ) {
                    ...
                    SetLowColor( (nLastAttrib & ATTR_INVERSE) ?
                                GetAttrFgColor( nLastAttrib ) :
                                GetAttrBgColor( nLastAttrib ) );
                } else {
                    SetHighColor( 0, 0, 0 );
                    SetLowColor( 200, 100, 200 );
                }
                int nStrLen = x - nStartX + 1;
                char zBuffer[nStrLen*3];
                
                const char* pzString = &pcLine->m_pCharMap[nStartX];

                if ( nLastAttrib & ATTR_HIGH_CHAR ) {
                    int nUTF8Len = 0;
                    for ( int i = 0 ; i < nStrLen ; ++i ) {
                        int nChar = (pzString[i]&0x80) ? uint8(pzString[i]) : (int(pzString[i])) | 0x2500;
                        int nChLen = unicode_to_utf8( zBuffer + nUTF8Len, nChar );
                        nUTF8Len += nChLen;
                    }
                    pzString = zBuffer;
                    nStrLen  = nUTF8Len;
                ...
                if ( nLastAttrib & ATTR_UNDERLINE ) {
                    StrokeLine( BPoint( nStartX * m_cCharSize.x, (y + 1) * m_cCharSize.y - 1 ),
                              BPoint( (nStartX + x - nStartX + 1 ) * m_cCharSize.x, (y + 1) * m_cCharSize.y - 1 ) );
                }
            }
            x += nSpaceCnt;
            nSpaceCnt = 0;
...
void TermView::RenderLine( int y, int nAscender, bool bOnlyModified )
{
    ...
    if ( NULL == pcLine ) {
        printf( "RenderLine(%d, %d, %d) failed to get line buffer\n", y, nAscender, bOnlyModified );
        return;
    }
    if ( NULL == pcPrevLine ) {
        printf( "RenderLine(%d, %d, %d) failed to get prev line buffer\n", y, nAscender, bOnlyModified );
        return;
    }
    ...
    else
    {
        for( x = 0 ; x <= m_cCurCharMapSize.x ; ++x )
        {
            ...
            {
                case 0:
                    if ( pcLine->m_pCharMap[x] != pcPrevLine->m_pCharMap[x] ||
                         pcLine->m_pAttribMap[x] != pcPrevLine->m_pAttribMap[x] )
                    {
                        x1 = x;
                        nState = 1;
                    }
...
void TermView::RefreshDisplay( bool bAll )
{
    ...
    if ( NULL == m_pBuffer )
	{
        printf( "Refresh called with m_pBuffer == NULL!!\n" );
        return;
    }
    ...
    if ( m_bModified || bAll )
    {
        ...
        if (m_cLastCsrPos != IPoint( m_cCsrPos.x, m_cCsrPos.y + m_nScrollPos ) )
        {
            ...
            if ( m_cLastCsrPos.x >= 0 && m_cLastCsrPos.x < m_cCurCharMapSize.x &&
                 m_cLastCsrPos.y >= 0 && m_cLastCsrPos.y < m_cCurCharMapSize.y )
            {
                if ( m_cLastCsrPos.x >= 0 && m_cLastCsrPos.x < m_cCurCharMapSize.x &&
                     m_cLastCsrPos.y >= 0 && m_cLastCsrPos.y < m_cCurCharMapSize.y )
                {
        ...
        if ( g_bDontRender )
		{
            printf( "PANIC! RefreshDisplay() Called while g_bDontRender was true!\n" );
            return;
        }
            ...
            if ( m_nTotLineCnt + nSteps > m_nMaxLineCount )
            {
                ...
                else
                {
                    ...
                    for ( i = 0 ; i < nSteps ; ++i )
                    {
                        ...
                        m_pBuffer[ 0 ] = pcTmp;
                        pcTmp->Clear( 0, 100000, ' ', m_nCurAttrib );
                    }
        ...
        else
        {
            ...
            if ( y2 > y1 )
            {
                ...
                for ( i = 0 ; i < nSteps ; ++i )
                {
                    ...
                    memmove( m_pBuffer + 1 + nBottomOffset, m_pBuffer + nBottomOffset,
                             sizeof( *m_pBuffer ) * (y2 - y1 ) );
                      /*m_pBuffer[ y1 ]*/ m_pBuffer[ m_cCurCharMapSize.y - y2 - 1 ] = pcTmp;
                      pcTmp->Clear( 0, 100000, ' ', m_nCurAttrib );
                }
            }
        }
...
void TermView::SetCsr( int x, int y, bool bScroll )
{
    ...
    if ( bScroll )
    {
        if ( m_cCsrPos.y > m_nScrollBottom ) {
            Scroll( m_nScrollBottom/*m_cCurCharMapSize.y*/ - m_cCsrPos.y, m_nScrollTop, m_nScrollBottom );
        }
        if ( m_cCsrPos.y < m_nScrollTop ) {
            Scroll( m_nScrollTop - m_cCsrPos.y, m_nScrollTop, m_nScrollBottom );
//      Scroll( -m_cCsrPos.y, m_nScrollTop, m_nScrollBottom );
        }
    }
    if ( m_cCsrPos.x < 0 )                        m_cCsrPos.x = 0;
    if ( m_cCsrPos.x >= m_cCurCharMapSize.x )     m_cCsrPos.x = m_cCurCharMapSize.x - 1;

    if ( m_cCsrPos.y < 0 )                        m_cCsrPos.y = 0;
    if ( m_cCsrPos.y >= m_cCurCharMapSize.y )     m_cCsrPos.y = m_cCurCharMapSize.y - 1;
  
    m_bModified = true;
}


void        TermView::PutChar( uint nChar )
{
    if ( m_nCharSet == 0 || m_nCharSet == -1 )
    {
        ...
        else
        {
            ...
            if ( m_cCsrPos.x < 0 || m_cCsrPos.x >= m_cCurCharMapSize.x ||
                 m_cCsrPos.y < 0 || m_cCsrPos.y >= m_cCurCharMapSize.y ) {
                return;
            }
            TextLine* pcLineBuf = GetLineBuf( m_cCsrPos.y );
            if ( m_bInsertMode && m_cCsrPos.x < pcLineBuf->m_nMaxLength - 1 ) {
                memmove( pcLineBuf->m_pCharMap + m_cCsrPos.x + 1,
                         pcLineBuf->m_pCharMap + m_cCsrPos.x,
                         pcLineBuf->m_nMaxLength - m_cCsrPos.x - 1 );
                memmove( pcLineBuf->m_pAttribMap + m_cCsrPos.x + 1,
                         pcLineBuf->m_pAttribMap + m_cCsrPos.x,
                         (pcLineBuf->m_nMaxLength - m_cCsrPos.x - 1)*2 );
            }
            pcLineBuf->m_pCharMap[ m_cCsrPos.x ] = nChar;
            pcLineBuf->m_pAttribMap[ m_cCsrPos.x ] = m_nCurAttrib;
            if ( nChar & 0x80 ) {
                pcLineBuf->m_pAttribMap[ m_cCsrPos.x ] |= ATTR_HIGH_CHAR;
            }
            InvalidateLine( m_cCsrPos.y );
            m_cCsrPos.x++;
            m_bModified = true;
    ...
    else
    {
        ...
        switch( nChar )
        {
            ...
            case 'j':        m_nCharSet = -1; PutChar( 0x18 ); m_nCharSet = 1; break; // 0x2518 (RB)
            case 'k':        m_nCharSet = -1; PutChar( 0x10 ); m_nCharSet = 1; break; // 0x2510 (RT)
            case 'l':        m_nCharSet = -1; PutChar( 0x0c ); m_nCharSet = 1; break; // 0x250c (LT)
            case 'm':        m_nCharSet = -1; PutChar( 0x14 ); m_nCharSet = 1; break; // 0x2514 (LB)
            case 't':        m_nCharSet = -1; PutChar( 0x1c ); m_nCharSet = 1; break; // 0x251c (|-)
            case 'u':        m_nCharSet = -1; PutChar( 0x24 ); m_nCharSet = 1; break; // 0x2524 (-|)
            case 'q':        m_nCharSet = -1; PutChar( 0x00 ); m_nCharSet = 1; break; // 0x2500 (-)
            case 'x':        m_nCharSet = -1; PutChar( 0x02 ); m_nCharSet = 1; break; // 0x2502 (|)
/*
            ...
            */                
            ...
            case 15:
                ...
                break;
            default:
                m_nCharSet = -1; PutChar( nChar ); m_nCharSet = 1; break;
...
void TermView::EraseFromTopToCursor()
{
    ...
    for ( i = 0 ; i < m_cCsrPos.y ; ++i )
    {
        ...
        memset( &GetLineBuf( i )->m_pCharMap, ' ', m_cCurCharMapSize.x );

        for (j = 0 ; j < m_cCurCharMapSize.x ; ++j ) {
            GetLineBuf( i )->m_pAttribMap[ j ] = m_nCurAttrib;
        }
        InvalidateLine( i );
    }
    m_bModified = true;
...
/*        assert( count >= 0 ); */
    ...
    if( len > 0 )
    {
        memmove( &GetLineBuf( m_cCsrPos.y )->m_pCharMap[ m_cCsrPos.x ],
                 &GetLineBuf( m_cCsrPos.y )->m_pCharMap[ m_cCsrPos.x + count ], len );
                
        memmove( &GetLineBuf( m_cCsrPos.y )->m_pAttribMap[ m_cCsrPos.x ],
                 &GetLineBuf( m_cCsrPos.y )->m_pAttribMap[ m_cCsrPos.x + count ], len * 2 );
                                                 
    }
    memset( &GetLineBuf( m_cCsrPos.y )->m_pCharMap[ m_cCurCharMapSize.x - count ], ' ', count );

    int        i;
        
    for ( i = 0 ; i < count ; ++i ) {
        GetLineBuf( m_cCsrPos.y )->m_pAttribMap[ (m_cCurCharMapSize.x - count ) + i] = m_nCurAttrib;
    }
...
/*        assert( count >= 0 ); */
    ...
    if( len > 0 )
    {
        memmove( &GetLineBuf( m_cCsrPos.y )->m_pCharMap[ m_cCsrPos.x + count ],
                 &GetLineBuf( m_cCsrPos.y )->m_pCharMap[ m_cCsrPos.x ], len );
                
        memmove( &GetLineBuf( m_cCsrPos.y )->m_pAttribMap[ m_cCsrPos.x + count ],
                 &GetLineBuf( m_cCsrPos.y )->m_pAttribMap[ m_cCsrPos.x ], len );
                                                 
    }
    memset( &GetLineBuf( m_cCsrPos.y )->m_pCharMap[ m_cCsrPos.x ], ' ', count );

    int        i;
        
    for ( i = 0 ; i < count ; ++i ) {
        GetLineBuf( m_cCsrPos.y )->m_pAttribMap[ m_cCsrPos.x + i ] = m_nCurAttrib;
    }
...
void TermView::InvalideEsc()
{
    for ( int i = 0 ; i < m_nEscStringSize ; ++i ) {
        if ( m_zEscString[i] == 0x1b ) m_zEscString[i] = '?';
    }
...
//                                MoveCsr( 0, -nDefault );
            ...
            case 'H':        // Move to row r, column c.
                if ( 0 == argc ) {
                    ...
                    printf( "ESC[r;nH sendt width %d numbers!\n", argc );
                }
                break;
            case 'I':        // Tab
                PutChar( '\t' );
                ...
                if ( 1 == argc )
                {
                    switch( argv[0] )
                    {
                        case 4:        // Insert mode
                            m_bInsertMode = true;
//              if ( g_nDebugLevel > 2 ) {
//                printf( "Insert mode not supported\n" );
//              }
                            break;
                        default:
                            InvalideEsc();
