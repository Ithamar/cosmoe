...
{
public:
	TermView( BRect cFrame, const char* pzTitle, uint32 nResizeMask, uint32 nFlags );
	~TermView();
	...
	void        		FrameResized( float inWidth, float inHeight );
	...
	BPoint       GetGlyphSize() const { return( m_cCharSize ); }
	IPoint PixToChar( const BPoint& cPos ) {
		return( IPoint( cPos.x / m_cCharSize.x, ((cPos.y >= 0) ? cPos.y : (cPos.y - m_cCharSize.y))  / m_cCharSize.y ) );
	}
	BPoint CharToPix( const IPoint& cPos ) {
		return( BPoint( cPos.x * m_cCharSize.x, cPos.y * m_cCharSize.y ) );
	}
