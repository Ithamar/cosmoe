...
ConfigView::ConfigView(BRect rect, const char *name, int mode, Prefs *prefs) :
	...
	if (mode == NORMAL_WINDOW_MODE) {
		r.right = r.left + StringWidth("Fade colors") + 20;
		...
		r.right = r.left + StringWidth("Active color") + 20;
		...
		r.left = r.right + 5;
		r.right = r.left + StringWidth("Idle color") + 20;
		idle = new BRadioButton(r, "IdleColor", "Idle color",
			new BMessage(PRV_MINI_IDLE));
		bbox->AddChild(idle);

		r.left = r.right + 5;
		r.right = r.left + StringWidth("Frame color") + 20;
		...
		r.bottom = r.top + 15;
		r.left = 10;
		r.right = r.left + StringWidth("Width of icon:") + 5 + 30;
		char temp[10];
		...
		iconwidth->SetDivider(StringWidth("Width of icon:") + 5);
