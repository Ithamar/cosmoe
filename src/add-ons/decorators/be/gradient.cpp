...
Gradient::Gradient(rgb_color begin,rgb_color end,uint8 steps)
{
	this->steps = steps;
	this->color = new rgb_color[steps];
	...
	{
		color[s] = HSV( hBegin.hue + s*hDelta,
						hBegin.saturation + s*sDelta,
						hBegin.value + s*vDelta ).GetRGB();
	}
}
